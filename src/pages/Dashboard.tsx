...
import { getItems } from "@/lib/wardrobe-store";
import { CATEGORIES } from "@/lib/types";
import { Shirt, TrendingUp, Droplets, CalendarDays } from "lucide-react";
    ...
    <div className="space-y-8">
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className="bg-secondary text-secondary-foreground px-4 py-2 rounded-lg text-sm font-medium hover:bg-accent hover:text-accent-foreground transition-colors"
              >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { label: "Laundry Tracker", desc: "Track what needs washing", icon: Droplets, to: "/laundry" },
        ].map((action) => (
          <motion.button
            key={action.to}
            ...
            className="bg-card border rounded-lg p-5 text-left hover:shadow-md transition-shadow"
          >
